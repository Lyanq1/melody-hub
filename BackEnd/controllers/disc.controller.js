...
export const countVinylByIso2 = async (req, res) => {
  try {
    // Find discs whose name contains "ĐĨA THAN"
    const discs = await Disc.find({ name: /ĐĨA THAN/i }, '_id name iso2')

    // Group by iso2, collect count and disc info
    const result = discs.reduce((acc, disc) => {
      const iso2 = disc.iso2 || 'unknown'
      if (!acc[iso2]) {
        acc[iso2] = { count: 0, discs: [] }
      }
      acc[iso2].count += 1
      acc[iso2].discs.push({ _id: disc._id, name: disc.name })
      return acc
    }, {})

    res.json(result)
  } catch (error) {
    console.error('Error counting vinyl by iso:', error)
    res.status(500).json({ message: 'Error counting vinyl by iso', error: error.message })
  }
}
